...
// import { testSmtpServer } from "./lib/test-smtp-server";
 ...
 * @returns message id
 */
async function sendMail(mail: SendMailOptions,
  smtpOptions: ISMTPOptions): Promise<string> {
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    transporter.sendMail(mail, (error, info: { messageId: string })
      : void => {
      if (error) {
        reject(error);
      } else {
