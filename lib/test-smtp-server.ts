...
export class eMail {
  ...
  length = 0;
  constructor(envelope: SMTPServerEnvelope) {
  ...
  public constructor(options?: testSmtpServerOptions | undefined) {
    ...
    this.server = new SMTPServer({
      ...
      onData(stream, session, callback) {
        ...
        stream.pipe(writer);
        stream.on("end", () => {
          const email = new eMail(session.envelope);
          email.buffer = Buffer.concat(buffers);
          // buf.pipe(process.stdout);
          that.emails.unshift(email);
          that.debug(JSON.stringify(email, undefined, 2));
          callback();
        });
      },
      secure: true,
